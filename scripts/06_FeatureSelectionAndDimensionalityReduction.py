...
sc.pl.pca_overview(sce, keys=['sum', 'n_genes_by_counts', 'SampleGroup', 'SampleName', 'pct_counts_mt'])

# Choosing PCs

# Elbow method
from kneed import KneeLocator
kn = KneeLocator(range(1, len(percent_var) + 1), percent_var, curve='convex', direction='decreasing')
...
for perplexity in [5, 500]:
    sc.tl.tsne(sce, n_pcs=10, perplexity=perplexity, random_state=123, key_added=f'tsne_perplex{perplexity}')
...
for n_neighbors in [5, 500]:
    sc.pp.neighbors(sce, n_neighbors=n_neighbors, n_pcs=10, key_added=f'neighbors_{n_neighbors}')
    sc.tl.umap(sce, neighbors_key=f'neighbors_{n_neighbors}', key_added=f'umap_{n_neighbors}')
